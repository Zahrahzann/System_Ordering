...
<head>
    <meta charset="UTF-8">
    <title>Riwayat Work Order</title>
...
<body id="page-top">
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        <?php if (empty($items)): ?>
                                            <tr><td colspan="<?= $currentRole === 'customer' ? '6' : '5' ?>" class="text-center">Tidak ada riwayat untuk periode ini.</td></tr>
                                        <?php else: ?>
                                            ...
                                            <tr>
                                                ...
                                                <!-- Tombol "Pesan Lagi" (Hanya untuk Customer) -->
                                                <?php if ($currentRole === 'customer'): ?>
                                                    <td>
                                                        <a href="<?= $basePath ?>/customer/history/reorder/<?= $item['item_id'] ?>" class="btn btn-success btn-sm" onclick="return confirm('Item ini akan ditambahkan kembali ke keranjang Anda. Lanjutkan?')">
    ...
    <!-- Hanya muat Chart.js jika Admin -->
    <?php if ($currentRole === 'admin'): ?>
        <script src="<?= $basePath ?>/assets/vendor/chart.js/Chart.min.js"></script>
        <script>
        // Mengubah data PHP (array 1-12) menjadi data JS (array 0-11)
        const phpData = <?= json_encode(array_values($chartData)) ?>;
        
        Chart.defaults.global.defaultFontFamily = 'Nunito, -apple-system,system-ui,BlinkMacSystemFont,"Segoe UI",Roboto,"Helvetica Neue",Arial,sans-serif';
        Chart.defaults.global.defaultFontColor = '#858796';

        var ctx = document.getElementById("monthlyWoChart");
        var myLineChart = new Chart(ctx, {
          type: 'line',
          data: {
            labels: ["Jan", "Feb", "Mar", "Apr", "Mei", "Jun", "Jul", "Ags", "Sep", "Okt", "Nov", "Des"],
            datasets: [{
              label: "Jumlah Item Selesai",
              lineTension: 0.3,
            ...
            tooltips: {
              callbacks: {
                label: function(tooltipItem, chart) {
                  return 'Jumlah: ' + Number(tooltipItem.yLabel) + ' item';
                }
              }
            }
          }
