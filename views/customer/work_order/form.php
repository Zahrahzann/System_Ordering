        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            fileInput.addEventListener('change', function(e) {
                ...
                if (files.length > 0) {
                    ...
                    const defaultText = '<?= $isEditMode ? 'Upload file baru untuk mengganti file lama' : 'Format: PDF, JPG, PNG (Max 10MB per file)' ?>';

                    display.innerHTML = `
                    <i class="fas fa-check-circle" style="color: #28a745;"></i>
                    <p><strong>${files.length} file terpilih</strong></p>
                    <p class="file-info" title="${fileNames}">${fileNames}</p>
                `;
                }
            });
            ...
            emergencyCheckbox.addEventListener('change', toggleEmergencyType);
